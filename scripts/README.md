...
1. `soloLVLdev-{version}-1.x86_64.rpm` (Linux RPM)
2. `soloLVLdev-{version}-full.nupkg` (Windows NuGet package)
3. `soloLVLdev-{version}.Setup.exe` (Windows installer)
4. `soloLVLdev-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `soloLVLdev-darwin-x64-{version}.zip` (macOS Intel)
6. `soloLVLdev_{version}_amd64.deb` (Linux DEB)
