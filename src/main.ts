...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient("soloLVLdev", process.execPath, [
      path.resolve(process.argv[1]),
    ]);
  }
} else {
  app.setAsDefaultProtocolClient("soloLVLdev");
}
...
export async function onReady() {
  ...
  if (settings.enableAutoUpdate) {
    ...
    const host = `https://api.soloLVLdev.ai/v1/update/${postfix}`;
    logger.info("Auto-update release channel=", postfix);
    updateElectronApp({
      logger,
      updateSource: {
        type: UpdateSourceType.ElectronPublicUpdateService,
        repo: "smolboon/soloLVLdev",
        host,
      },
...
async function handleDeepLinkReturn(url: string) {
  // example url: "soloLVLdev://supabase-oauth-return?token=a&refreshToken=b"
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (parsed.protocol !== "soloLVLdev:") {
    dialog.showErrorBox(
      "Invalid Protocol",
      `Expected soloLVLdev://, got ${parsed.protocol}. Full URL: ${url}`,
    );
    return;
  }
  ...
  // soloLVLdev://soloLVLdev-pro-return?key=123&budget_reset_at=2025-05-26T16:31:13.492000Z&max_budget=100
  if (parsed.hostname === "soloLVLdev-pro-return") {
  ...
  // soloLVLdev://add-mcp-server?name=Chrome%20DevTools&config=eyJjb21tYW5kIjpudWxsLCJ0eXBlIjoic3RkaW8ifQ%3D%3D
  ...
  // soloLVLdev://add-prompt?data=<base64-encoded-json>
