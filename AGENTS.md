...
| [rules/chat-message-indicators.md](rules/chat-message-indicators.md) | Using `<soloLVLdev-status>` tags in chat messages for system indicators                                |
| [rules/product-principles.md](rules/product-principles.md)           | Planning new features, especially via `soloLVLdev:swarm-to-plan`, to guide design trade-offs           |
...
2. If the branch hasn't been pushed before, default to pushing to `origin` (the fork `smolboon/soloLVLdev`), then create a PR from the fork to the upstream repo (`dyad-sh/dyad`).
